...
```bash
gh release download v0.2.0 --repo RefireLab/gh-runnerd \
...
The VM image bake is built into the binary (`runner-image bake`): it downloads the Ubuntu 24.04 cloud image, installs Docker + the GitHub Actions runner + the guest agent inside a throwaway VM, and activates the result. Once per machine, 10-20 minutes.
