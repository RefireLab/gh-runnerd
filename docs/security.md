...
- **VM isolation.** iptables drops VM-to-VM traffic. Outbound NAT is allowed only from the runner bridge (`FORWARD ACCEPT` scoped to that iface) so VMs can reach GitHub when Docker/ufw set `FORWARD DROP`. Each VM has its own overlay disk, destroyed after the job.
