...
| `selftest dhcp FAILED` while dns/tcp443 pass | Another process owns UDP :67, so the daemon's DHCP server could not bind — the `dhcp server down` log line names it (commonly dnsmasq). Scope it away from the runner bridge (dnsmasq: `except-interface=br-ghrunnerd` or `bind-interfaces`) or stop it; serve rebinds automatically every 15s. |
