...
| Offline runners pile up in GitHub after crashes or restarts | GitHub keeps a dead JIT registration around for about a day. `serve` now deregisters a runner whenever its VM is destroyed, and sweeps leftover `<prefix>-N-N` registrations every `runner.cleanup_interval` (default 3m; a candidate must stay offline+untracked ≥2 min before deletion). One-shot purge: `gh-runnerd runners cleanup` (see `runners list`). |
| Runner still Idle in GitHub after the daemon was stopped | A graceful stop now deregisters its runners. After a hard kill GitHub keeps showing the dead runner Idle for several minutes until it notices the broken connection; the sweep catches it once GitHub flips it to Offline, or remove it immediately with `gh-runnerd runners cleanup --idle`. |
